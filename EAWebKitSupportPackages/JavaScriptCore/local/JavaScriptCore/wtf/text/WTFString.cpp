...
#else
	//+EAWebKitChange
	//10/17/2011 - In String::format(const char *format, ...)
	// Note by Arpit Baldeva: We use the same code that all other platforms use (except the ones listed above).
	// The problem here is that in some locales, a float say 0.5 gets converted to "0,5" here. Some parts of WebKit can't handle this.
	// This bug went on for 2 years. Qt checked in a fix early on that simply avoids locale dependent formatting here. Later, it was resolved by modifying
	// the code at the source location in various patches. WebKit communities ultimate plan is to kill this function but for now it should be safe to use.
	// We can't use eastl::string similar to QString for locale-independent formatting because it does not have similar functionality. In fact, it relies
	// on the application to provide the equivalent of vsnprintf.
	// Details - https://bugs.webkit.org/show_bug.cgi?id=18994
	//-EAWebKitChange
    va_list args;
    va_start(args, format);
...
{
#if (OS(WINDOWS) || defined(EA_PLATFORM_MICROSOFT)) && !PLATFORM(QT)
